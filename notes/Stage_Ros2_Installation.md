...
- Update the needed development packages and tools:
  ...
  - ```bash
      sudo apt-get install git cmake g++ libjpeg8-dev \
        libpng-dev libglu1-mesa-dev libltdl-dev libfltk1.3-dev
    ```
...
- Install stage as dependency for the stage_ros2 bridge: (only works manually on Ubuntu 24.04)
  ...
  - Use ```make -j<number_of_cores>``` to speed it up make (not recommended on wsl2)
...
- Make sure stage_ros2 finds the stage installation:
  ...
  - this path might be different on your system or not needed when installing in the same workspace
...
sudo apt-get install git cmake g++ libjpeg8-dev libpng-dev \
  libglu1-mesa-dev libltdl-dev libfltk1.3-dev
...
export CMAKE_PREFIX_PATH=/usr/local/lib/cmake:$CMAKE_PREFIX_PATH # maybe not needed
